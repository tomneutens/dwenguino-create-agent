...
import (
	...
	"arduino.cc/arduino-create-agent/tools"
...
func main() {
	...
	go func() {
		log.Println("Starting profiling server")
	...
	if *hibernate == false {
		...
		go func() {
			...
			extraOriginStr := "http://localhost:80, http://localhost, https://localhost:80, https://localhost, *"
			...
			r.Use(cors.Middleware(cors.Config{
				...
				RequestHeaders:  "Origin, Accept-Language, Authorization, Content-Type, Content-type, Content-Length, Accept-Encoding, X-CSRF-Token, Authorization, X-Requested-With, Accept, Cache-Control, Referer, Access-Control-Request-Method, User-Agent, Connection, Host",
				ExposedHeaders:  "",
			...
			r.GET("/listPorts", listPortsHandler)
			r.POST("/uploadToDwenguino", uploadToDwenguinoHandler)
