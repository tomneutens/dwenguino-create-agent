...
type ToSourceLogger struct {
	buffer bytes.Buffer
}

func NewToSourceLogger() *ToSourceLogger {
	return &ToSourceLogger{}
}

func (s *ToSourceLogger) printWithoutFormatting(w io.Writer, level string, format string, a []interface{}) {
	a = append([]interface{}(nil), a...)
	for idx, value := range a {
		typeof := reflect.Indirect(reflect.ValueOf(value)).Kind()
		if typeof == reflect.String {
	...
	//fmt.Printf("logging: ===%s ||| %s ||| %s", level, format, a)
	//fmt.Println("")
	newLogLine := fmt.Sprintf("===%s ||| %s ||| %s\n", level, format, a)
	s.buffer.WriteString(newLogLine)
}

func (s *ToSourceLogger) PopLog() string {
	buff := s.buffer.String()
	s.buffer.Reset()
	return buff
}

func (s *ToSourceLogger) Fprintln(w io.Writer, level string, format string, a ...interface{}) {
	s.printWithoutFormatting(w, level, format, a)
}

func (s *ToSourceLogger) Println(level string, format string, a ...interface{}) {
	s.printWithoutFormatting(os.Stdout, level, format, a)
}

func (s *ToSourceLogger) Name() string {
	return "machine"
}
