...
func ExecRecipe(properties properties.Map, recipe string, removeUnsetProperties bool, echoCommandLine bool, echoOutput bool, logger i18n.Logger) ([]byte, error) {
	...
	// Add a buffer so the command line output can also be logged
	...
	// log buffer and send it to stdout and Stderr
	if echoOutput {
		// fmt.Fprintf(os.Stdout, outbuf.String())
		logger.Println("Progress {0}", outbuf.String())
	}
	//fmt.Fprintf(os.Stderr, errbuf.String())
	if errbuf.String() != "" {
		logger.Println("Error", errbuf.String())
	}
	errbuf.Reset()
	outbuf.Reset()
